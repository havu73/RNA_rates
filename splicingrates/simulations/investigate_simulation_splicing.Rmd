---
title: "investigate_simulation_splicing"
...
```{r}
library(stats)
# parameters that I am supposed to set
DIR = 'trial'
D_dists = c(5)
expression_levels = c(5)
```
...
formatreads <- function(reads, intron, start_pos, U_dist, rl){
  ...
  junction = (reads$start > -rl + 10) & (reads$start <= -9) & start_pos$spliced # since reads$start are the start of the fragment relative of the begining of the intron, this means that any fragments that before the begining of the intron will have negative start here, and we want to find fragments that have negative start, but not too negative (smaller than -read_length + 10). Number 10 is just a subjective number here, because we also do not want reads that are lying mostly on one side of the junction. 
  ...
  cigar.vec[is.junction] = paste(as.character(-reads$start[is.junction]), 'M', as.character(intron), 'N',
                                 as.character(rl+reads$start[is.junction]), 'M', sep="")  # the first segment is matched with the sequence for the intron, then the intron is skipped (N), and the remaining part of the read is match with the sequence right after the end of the intron
...
{
  # Inputs: 
  #   intron - length of the intron in kb. ex: 0.04
  #   exon - length of downstream exon in kb. ex: 0.3
  #   U_dist - distance from the TSS to the beginning of the intron, in kb. ex: 0.5
  #   D_dist - distance from the end of the exon to the PAS (polyA site) in kb. ex: 5
  #   labeling - the length of the labeling period in minutes. ex: 5
  #   half_life - the half-life time in minutes. ex: 0.2
  #   expression_level - the expression level in TPM. ex: 5
  #   n_millions - the total number of millions of transcripts to consider. ex: 100
  #   transcription_rate - rate of transcription in kb/min. ex: 1.5
  ...
  start_pos$spliced = spliced[start_pos$transcript] # for each read, report whether or not this read is from a transcript that has been spliced or not
  ...
  for (D_dist in D_dists){
    for (expression_level in expression_levels){
      for (half_life in half_lives){
        ...
        for (intron in introns){
          cat(paste0(intron," - "))
          if(intron %% 1 == 0){ flush.console() } # for every even intron, print to the terminal the progress
          print(paste(intron, exon, U_dist, D_dist, labeling, half_life, expression_level, n_millions, transcription_rate))
          exit(1)
...
#                     length.out = expression_level*n_millions))
end_sites = U_dist + sample.int(intron + exon + D_dist + labeling*transcription_rate,
                                expression_level*n_millions, replace = TRUE) # simulate the end points of transcript. Here, we only simulate transcripts that go at least as far as the begining of the intron, because those are the trancripts that may be potentially spliced at this intron. 
...
# up until here, everything will be consistent with the paper if end_sites do not include U_dist
...
# Get the reads from the transcripts and map them to the gene
rl = 50 # read length
start_pos = get_reads(lengths) # a dataframe of columns: transcript, start --> each row correspond to a fragment of each transcript
toss_distance = 300
start_pos = start_pos[start_pos$start > toss_distance,]
start_pos$spliced = spliced[start_pos$transcript] # for each read, we want to keep record of whether this read is generated from a transcript that has been spliced or not.

#reads = t(data.frame(row.names = c('start', 'match')))
reads = data.frame(start = start_pos$start - U_dist +
                     intron*start_pos$spliced*(start_pos$start > U_dist)) 
# up until here, for each read, start is relative to the start of the intron (0)
# if fragment is from a spliced transcript and the start point of this fragment is supposedly beyond the beginning of the intron, then the intron is spliced out from this fragment, 
# which mean the start point of this fragment has to be update,
# however, I wonder if we can modify this such that the start point of this fragment after splicing is just the end the of intron instead of reads$start + intron_length
