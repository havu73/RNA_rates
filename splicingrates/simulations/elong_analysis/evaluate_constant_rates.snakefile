...
big_folder = '/gladstone/engelhardt/lab/hvu//RNA_rates/elong_rates/pair_end_reads/oct_24/constant/'
design_fn = f'{big_folder}/design_matrix.csv'
def num_lines_in_file(fn):
    df = pd.read_csv(fn, header=0, index_col=None, sep='\t', dtype={'constant_elong_rate': str})
    return df.shape[0]
...
# remove 17 from the list
rule all:
    input:
        expand(os.path.join(big_folder,  'result_{job_idx}', 'pred_h.csv.gz'), job_idx = jobs_to_care)
...
0            2          10              -1              -1           -1     9  (0.06, 0.4, 0.06)    (0.3, 2.0, 0.3)  splice_est/spliceH_est/noReads/result_0.csv
    ...
    for colname, value in row.items():
        ...
        elif type(value) == boolean and value:
